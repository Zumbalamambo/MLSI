...
def noExtractCompare(classifiers):
        # Get data, n_bands=4
    ...
    W=dataset[2]
    n_bands=dataset[3]
    org_data=art.tif2vec(dataset[0])

    X_train=org_data.reshape(-1,n_bands)
    print("shape of training data: ",X_train.shape)
    
    for clf_name,clf in classifiers.items():
        clf_name="no_extraction_"+clf_name
        ...
        print(y_train_pred.shape,y_train_scores.shape,outlier_result.shape,score_result.shape)
        #save the outlier detection result as .tif and .shp file
        outlier_save_path="C:\\Users\\DELL\\Projects\\MLS_cluster\\image-v2-timeseries\\Pyod_Algorithms\\compare without extraction"
        ...
        with open(outlier_save_path+'/'+"no_extraction_Outlier Detection Algorithms Running Time.txt", 'a') as f:
...
def Compare(classifiers):
    ...
    for clf_name,clf in classifiers.items():
        ...
        #save the outlier detection result as .tif and .shp file
        outlier_save_path="C:\\Users\\DELL\\Projects\\MLS_cluster\\image-v2-timeseries\\Pyod_Algorithms\\compare with extraction"
        DataProcess.int_to_csv(outlier_save_path,img,outlier_result,clf_name+"_outliers")
        GeoProcess.getSHP(norm_img_path,img,outlier_save_path,clf_name+"_outliers",outlier_result)
...
if __name__ == "__main__":
    ...
    classifiers = {
                ...
                'AutoEncoder': # FIXME: can't run? maybe the other program occupy the GPU
                  AutoEncoder(epochs=4, hidden_neurons=[4,2,4],contamination=outliers_fraction),
                # 'Feature Bagging':
